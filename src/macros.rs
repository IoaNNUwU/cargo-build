...
/// using `format!` macro syntax for each argument.
///
/// ```rust
...
/// ```
/// 
/// see [`rerun_if_changed` function](`crate::functions::rerun_if_changed`) if you dont want to 
/// use macros.
///
    ...
    ( $($($arg:tt),*);* ) => {{
        $crate::build_out::CARGO_BUILD_OUT.with(|out| {
            ...
            $(
                ...
                writeln!(out, $($arg),*).expect("Unable to write to CARGO_BUILD_OUT");
            )*
        });
    }};
}
...
///
/// Supports variable number of arguments. They are separated by `;` to allow interpolation
/// using `format!` macro syntax. You can use `,` separated format string with list of arguments.
/// 
/// ```rust
